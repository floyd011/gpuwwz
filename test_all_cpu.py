...
print(f"   Single 4-core max difference: {max_diff_single_4core:.2e}")
...
print(f"{'Multi-CPU (4 cores)':<20} {fourcore_time:<12.2f} {single_time/fourcore_time:<10.1f}x")
...
try:
    ...
    axes[1, 1].set_title(f'Single vs 4-core diff\\nmax: {diff2.max():.2e}')
    axes[1, 1].set_xlabel('Tau')
    axes[1, 1].set_ylabel('Frequency')
    plt.colorbar(im2, ax=axes[1, 1])
    
    # Performance bar chart
    methods = ['Single CPU', 'Multi-CPU (all)', 'Multi-CPU (4)']
